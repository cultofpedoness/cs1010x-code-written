...
def merge_lists(all_lst):
    ...
    while all_lst and all_lst != [[]]:
        ...
        for i in range(len(all_lst)):
            if all_lst[i][0]<current:
                current = all_lst[i][0]
                index = i
        result.append(current)
        all_lst[index].remove(all_lst[index][0])
    return result
...
def merge(lists, field):
    ...
    while lists and lists != [[]]:
        ...
        for lst in lists:
            if lst != []:
                next_list.append(lst)
        if next_list == []:
            break
        current = next_list[0][0]
        index = 0
        for i in range(len(next_list)):
            if field(next_list[i][0])<field(current):
                current = next_list[i][0]
                index = i
        result.append(current)
        next_list[index].remove(next_list[index][0])
        lists = next_list
    return result
...
def merge_sort(lst, k, field):
    def helper(entry):
        new_list = []
        if len(entry) <= k:
            return merge([[x] for x in entry], field)
        else:
            num = len(entry)//k
            for i in range(k):
                if i != k-1:
                    new_list.append(helper(entry[i*num:(i+1)*num]))
                else:
                    new_list.append(helper(entry[i*num:]))
            return merge(new_list, field)
