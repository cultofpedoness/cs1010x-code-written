...
#=> Output: True
#=> Explain the outputs:
#=> For tuple, as it is immutable, a new tuple is created when the original
#   tuple was spliced and concatenated. The original tuple remains unchanged.
#   As such, not only does the resulting tuple have a different identity, the
#   contents are different as well, resulting in both the identity and
#   equivalence check to be False.
#=> On the other hand, for list, it is mutable, as such the list that was
#   passed in was changed, aka the global list was changed, and the same list
#   was then returned after the insertion. As such, both the identity and
#   equivalence check would return True.
